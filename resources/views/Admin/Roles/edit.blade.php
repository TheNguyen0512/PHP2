...
@section('content')
    ...
    @include('Admin.pages.content-header', ['name' => 'Roles' , 'key' => 'edit'])
    <!-- /.content-header -->
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <div class="panel-body">
                            <form  method="post" action="{{route('roles.update', ['id'=>$roleCheckd->id])}}" enctype="multipart/form-data">
                            @csrf
                            <!-- name -->
                                <div class="form-group">
                                    ...
                                    <input require="true" type="text" class="form-control  @error('name') is-invalid @enderror"
                                           id="name" name="name"
                                           placeholder="Enter slider name" value="{{$roleCheckd->name}}">
                                </div>
                                @error('name')
                            ...
                            <!-- display_name -->
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control" id="exampleFormControlTextarea5" rows="3" name="display_name">{{$roleCheckd->display_name}}</textarea>
                                </div>
                                @error('email')
                                <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                                                        ...
                                                        <h4 class="card-title">
                                                            <div class="form-check">
                                                                <input class="form-check-input checkbox_childent"
                                                                       {{$permissionChecked->contains('id', $itemChildent->id)?'checked':''}}
                                                                       type="checkbox" name="permission_id[]" value="{{$itemChildent->id}}">
                                                                <label class="form-check-label" for="flexCheckChecked">
                                                                    {{$itemChildent->name}}
                                                                </label>
                                                            </div>
